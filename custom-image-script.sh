...
if aws ecr get-login-password --region ap-southeast-1 | docker login --username AWS --password-stdin 503561440800.dkr.ecr.ap-southeast-1.amazonaws.com; then
  ...
  docker build -t custom-starai-lck50:"${IMAGE_TAG}" .

  # this tags the image custom-starai-lck50 to the remote ECR with the tag latest
  docker tag custom-starai-lck50:"${IMAGE_TAG}" 503561440800.dkr.ecr.ap-southeast-1.amazonaws.com/custom-starai-lck50:"${IMAGE_TAG}"

  # perform the push to the ECR with the image tag latest that was set above
  docker push 503561440800.dkr.ecr.ap-southeast-1.amazonaws.com/custom-starai-lck50:"${IMAGE_TAG}"
